...
export const getProductUnits = async (req, res) => {
  try {
    ...
    if (is_sales_unit !== undefined)
      where.is_sales_unit = is_sales_unit === "true";
    if (is_purchase_unit !== undefined)
      where.is_purchase_unit = is_purchase_unit === "true";
...
export const createProductUnit = async (req, res) => {
  try {
    ...
    const productUnit = await ProductUnit.create({
      ...
      is_purchase_unit:
        is_purchase_unit !== undefined ? is_purchase_unit : true,
      price_modifier,
    });
    ...
    await createAuditLog({
      ...
      changes: { after: createdProductUnit.toJSON() },
      req,
    });
    res.status(201).json({
      success: true,
...
export const updateProductUnit = async (req, res) => {
  try {
    ...
    const beforeUpdate = productUnit.toJSON();
    ...
    await createAuditLog({
      ...
      userId: req.user?.id,
      userEmail: req.user?.email,
      changes: { before: beforeUpdate, after: updatedProductUnit.toJSON() },
      req,
    });
    res.json({
      success: true,
  ...
  try {
    ...
    if (productUnit.is_base_unit) {
      return res.status(400).json({
        success: false,
        message:
          "No se puede eliminar la unidad base. Asigna otra unidad como base primero.",
      });
    }
    const beforeDelete = productUnit.toJSON();
  ...
  try {
    ...
    for (const productUnit of productUnits) {
      await createAuditLog({
        ...
        userId: req.user?.id,
        userEmail: req.user?.email,
        changes: { after: productUnit.toJSON() },
        req,
      });
    }
