      ...
      if (recipeData.recipes && recipeData.recipes.length > 0) {
        ...
        const enrichedRecipes = await Promise.all(
          recipeData.recipes.map(async (recipe) => {
            try {
              const rawMaterial = await rawMaterialService.getRawMaterialById(
                recipe.raw_material_id
              );
              console.log("Materia prima cargada:", rawMaterial); // Debug
              return {
                ...recipe,
              ...
              console.error(
                `Error al cargar materia prima ${recipe.raw_material_id}:`,
                error
              );
              return {
                ...recipe,
                rawMaterial: { name: "Error al cargar", description: "" },
              };
            }
          })
        );

        console.log("Recetas enriquecidas:", enrichedRecipes); // Debug
